	...
	if product != null:
		if ingredients_counter>=ingredients[1]:
			if timer<=duration:
				...
				if (new_ressource==null or (new_ressource is Node2D and new_ressource.position.distance_to(self.position)>64)) and get_parent().get_parent().get_node("Ressources").use_energy(energy_consumption) and use_ingredients():
					timer-=duration
